...
selected_b_field = -11.9  # Change this to the desired B field value
...
if len(b_field_index) > 0:
    ...
    plt.plot(frequencies, selected_log_magnitude_s21, label=f"B field = {selected_b_field} T", color='b')
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Log Magnitude of S21 (dB)")
    plt.title(f"S21 vs Frequency for B field = {selected_b_field} T")
    plt.legend()
    plt.grid(True)
    plt.show()
else:
