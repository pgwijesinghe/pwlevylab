...
sweep_folder_path = r"G:\.shortcut-targets-by-id\0B8-gGFa6hkR4XzJJMDlqZXVKRk0\ansom\Data\THz 1\SA40287C.20240828\04 - BG Sweep"

wiring_dict = {'I+': 1, 'I-': 3, 'dV1': 2}

index = r"G:\.shortcut-targets-by-id\0B8-gGFa6hkR4XzJJMDlqZXVKRk0\ansom\Data\THz 1\SA40287C.20240828\04 - BG Sweep\SA40287C.20240828.000009.tdms_index"
...
def plot_IV_sweep(type, datafile, savetoCSV=False):
    ...
    with TdmsFile.open(tdms_file_path) as tdms_file:
        ...
        if type == '2T':
            plt.plot(Vsource, Iminus, label=str(sweep_value))
            plt.xlabel('Source Voltage (V)')
            plt.ylabel('Current (A)')
        elif type == '4T':
            plt.plot(dV, Iminus, label=str(sweep_value))
            plt.xlabel('Source Voltage (V)')
            plt.ylabel('Current (A)')
